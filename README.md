...
## Fast Paper Contract And Optional Editable PPTX
...
Automatic selection includes dedicated `dense-multiframe`, `multimodal`,
`feedback`, and `branch` templates. Dense task/model/data-engine overviews use
`dense-multiframe`; multiple modality inputs converging into one shared
representation use `multimodal`;
...
Image-2 generation uses a 120-second default request timeout configured by
`RFS_IMAGE2_TIMEOUT`. A network timeout is not blindly retried; retryable HTTP
