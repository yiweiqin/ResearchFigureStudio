...
When a deadline is active and multiple pages need local OCR, pages run in isolated worker processes. Completed pages are preserved, while workers still running at the reserved cutoff are terminated and recorded with `timed_out=true`. This prevents a slow ONNX recognition call from holding the fast workflow beyond its parsing budget or leaving background OCR processes behind.
