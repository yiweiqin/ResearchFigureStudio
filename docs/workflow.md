...
6. Create the style sheet before any image prompt is written.
7. Create `layout_plan.json`; use VLM only for normalized coordinate estimation when enabled.
8. Create `figure_program.json`; this is the only layout source for the PPT compiler.
9. Write `reference_slot_prompt_brief.json`; it records what each local reference slot does and what paper concept it carries.
10. Write `slot_prompt_plan.json`; default `--prompt-plan-mode vlm` calls the model for each slot and produces `image_prompt_core`. Use `--prompt-plan-workers` to run these per-slot VLM calls in parallel.
11. Generate multiple image candidates per slot from the model-planned prompt core.
12. Select assets by fill, margin, ratio, and cutoff metrics; never use semantic cropping.
13. Review selected assets with heuristic or VLM review.
14. Compile `editable_composition.pptx` deterministically from the figure program.
15. Export PDF/PNG for review where possible.
16. Run visual critic against the reference image and rendered output.
17. Write alignment review and critic report.
18. Run `rfs validate` before delivery.
...
```powershell
rfs make-framework --paper "C:\path\paper.pdf" --reference "C:\path\reference.png" --out D:\ResearchFigureStudio\output\offline_check --asset-mode placeholder --locator-mode heuristic --control-localizer-mode heuristic --prompt-plan-mode heuristic --slot-count 36 --candidates-per-slot 3 --asset-review-mode heuristic --critic-mode heuristic --json
